<?php
	...
	//Consulta
	$sql = "select * from (select nofactura, fecha, usuario, codcliente, totalfactura from factura order by nofactura desc limit 200) t order by t.nofactura; ";
	...
	$objPHPExcel->getProperties()->setCreator("Josthon")->setDescription("Reporte de Ventas");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE  DE VENTAS ');
	$objPHPExcel->getActiveSheet()->mergeCells('B3:D3');

	$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'NOFACTURA');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D6', 'CODCLIENTE');
	...
	while($rows = mysqli_fetch_array($resultado)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['nofactura']);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['fecha']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['usuario']);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['codcliente']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $rows['totalfactura']);
	...
	$title = new PHPExcel_Chart_Title('GRAFICO DE VENTAS'); // Set upYThe axis labels
