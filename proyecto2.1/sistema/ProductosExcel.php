<?php
	...
	//Consulta
	$sql = "select * from (select codproducto, descripcion, proveedor, precio, existencia from producto order by codproducto desc limit 200) t order by t.codproducto; ";
	...
	$objPHPExcel->getProperties()->setCreator("Josthon")->setDescription("Reporte de de Productos");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'CODPRODUCTO');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E6', 'EXISTENCIA');


	//Recorremos los resultados de la consulta y los imprimimos
	while($rows = mysqli_fetch_array($resultado)){

		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['codproducto']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['precio']);
	...
	$title = new PHPExcel_Chart_Title('GRAFICO DE PRODUCTOS'); // Set upYThe axis labels
