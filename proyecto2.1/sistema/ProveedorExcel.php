<?php
	...
	//Consulta
	$sql = "select * from (select codproveedor , proveedor, contacto, telefono, direccion from proveedor order by codproveedor  desc limit 200) t order by t.codproveedor ; ";
	...
	$objPHPExcel->getProperties()->setCreator("Josthon")->setDescription("Reporte de de Proveedores");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'CODPROVEEDOR');
	...
	while($rows = mysqli_fetch_array($resultado)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['codproveedor']);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['proveedor']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['contacto']);
	...
	$title = new PHPExcel_Chart_Title('GRAFICO DE PROVEEDOR'); // Set upYThe axis labels
